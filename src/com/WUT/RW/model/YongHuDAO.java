	...
	public boolean  deleteYongHu(int yonghudianhua) {
		boolean result=false;
		  try {
				...
				Connection  connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/zhaopingwang?serverTimezone=UTC","root","root");
				QueryRunner run = new QueryRunner();
				int count=run.update(connection,"delete from yonghu where yonghudianhua=?",yonghudianhua);
				result=count>0?true:false;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public boolean  updateYongHu(Yonghu y) {
		boolean result=false;
		  try {
				...
				Connection  connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/zhaopingwang?serverTimezone=UTC","root","root");
				QueryRunner run = new QueryRunner();
				int count=run.update(connection,"update yonghu set yonghuming=?,yonghudianhua=?,yonghuxingbie=?,yonghudizhi=?,yonghuemaildizhi=? where yonghudianhua=?",y.getYonghuming()
